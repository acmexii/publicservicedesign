<template>
    <v-card outlined>
        <v-card-title>
            Investigation # {{item._links.self.href.split("/")[item._links.self.href.split("/").length - 1]}}
        </v-card-title>
        ...
        <v-card-text>
            ...
            <div>
                <String label="민원번호" v-model="item.민원번호" :editMode="editMode" @change="change" />
            </div>
        </v-card-text>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/investigations/' + params.id))
            if(temp.data) {
                me.item = temp.data
            }
        },
