<template>
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-title v-if="value._links">
            사실조사 # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            사실조사
        </v-card-title >        
        ...
        <v-card-text>
            ...
            <String label="설명" v-model="value.description" :editMode="editMode" :inputUI="'TEXTAREA'"/>
        ...
        methods: {
            ...
            async save(){
                try {
                    ...
                    if(!this.offline) {
                        if(this.isNew) {
                            temp = await axios.post(axios.fixUrl('/investigations'), this.value)
                        } else {
